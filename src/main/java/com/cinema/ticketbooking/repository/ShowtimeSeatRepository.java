...
public interface ShowtimeSeatRepository extends JpaRepository<ShowtimeSeat, ShowtimeSeatId> {
         ...
         * Instead of fetching 1,000 expired objects into Java memory and saving them
         * one by one,
        ...
        @Query("SELECT ss FROM ShowtimeSeat ss WHERE ss.status = 'HELD' AND ss.holdUntil <= :now")
        List<ShowtimeSeat> findExpiredHolds(@Param("now") java.time.LocalDateTime now);

        /**
         * 3. NATIVE MYSQL QUERY: Finding Hot/Contiguous Seats (Optional
